...
with open("weather+{}.cvs".format(filename[:-4]),"w", encoding='utf8') as output:
    ...
    with open(filename, "r", encoding = 'utf8') as f:
        ...
        for row in reader:
            ...
            data = requests.get("https://api.weather.gov/points/{lat},{lon}".format(lat = row['Latitude'][:7], lon = row['Longitude'][1:10])).json()
            ...
            if seen.count((gridX, gridY, gridId)) == 0:
                ...
                date = data['properties']['updated'] # Last date forcast was updated.
                mdt = (data['properties']['periods'][0]['temperature'] + data['properties']['periods'][1]['temperature']) / 2   # Average of morning and evening temperatures.
